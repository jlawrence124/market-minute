...
export async function generatePodcastScript(context: string, tickers: string[]): Promise<string> {
  const prompt = `You are the host of 'Schwab Flash Cast', an expert financial news podcaster working at the company Charles Schwab. 
  
  You MUST NOT acknowledge yourself as host by name. Just deliver the podcast.
  ...
  **CRITICAL FORMATTING RULE FOR STOCK TICKERS:**
  ...
  1. A brief introduction for "Schwab Flash Cast", stating that you are powered by Charles Schwab and mention that this podcast has been compiled from securities that the listener owns. Also, include a disclaimer that this podcast was "created using generative AI", "should NOT be considered financial advice", "Please consult with your qualified Schwab advisor when making financial decisions"
  2. A segment for each security, making sure to use the special ticker formatting.
  3. A concluding remark.
  The script should be natural-sounding when read aloud. End the podcast with "That's your Schwab Flash Cast update for today. Thanks for tuning in."
